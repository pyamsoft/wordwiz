...
internal abstract class WordActivity<out T : WordProcessActivity> {
  ...
  @CheckResult abstract fun provideClassType(): Class<out T>
  ...
  private fun enable(context: Context, enable: Boolean, targetClass: Class<out T>) {
  ...
  @CheckResult
  private fun isEnabled(context: Context, targetClass: Class<out T>): Boolean {
    val name = ComponentName(context, targetClass)
